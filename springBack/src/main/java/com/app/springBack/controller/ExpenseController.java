...
public class ExpenseController {
    ...
    @PreAuthorize("@authServiceUser.isRightUser(#userId)")
    public ResponseEntity<ExpenseDto> postExpense(@PathVariable("id") int userId, @Valid @RequestBody ExpenseDtoInput expenseDtoInput) {
        return ResponseEntity
            .status(HttpStatus.CREATED)
            .body(this.expenseService.createExpenseFromDtoInput(userId, expenseDtoInput)); 
    }

    @PutMapping("/user/{id}/expense/{expenseId}")
    @PreAuthorize("@authServiceUser.isRightUser(#userId)")
    public ResponseEntity<ExpenseDto> putExpense(@PathVariable("id") int userId, @PathVariable("expenseId") int expenseId, @Valid @RequestBody ExpenseDtoInput expenseDtoInput) {
        return ResponseEntity
            .status(HttpStatus.CREATED)
            .body(this.expenseService.updateExpense(expenseId, expenseDtoInput));
    }
